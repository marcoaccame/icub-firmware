...
{   
    ...
    if(true == embot::core::binary::mask::check(eventmask, evtTick)) 
    {
        ...
        testduration();   
        signal->off();    
        embot::core::Time delta = embot::core::now() - n;
        
        embot::core::TimeFormatter tf1(delta);        
        embot::core::print("execution time is " + tf1.to_string());  
...
//    theleds.get(embot::hw::LED::one).wave(&ledwave); 
    ...
    embot::core::print("INIT: creating the main thread. it will receive one periodic tick event and one upon pressure of the blue button");  
